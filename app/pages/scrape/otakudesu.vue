...
async function handleScrape() {
  ...
  try {
    const data = await $fetch(`/api/scrape/otakudesu?url=${encodeURIComponent(targetUrl.value)}`)
    if (data.success) {
...
<template>
  <div class="max-w-4xl mx-auto px-4 py-8">
    <!-- Header -->
    <div class="mb-6">
      <h1 class="text-xl font-bold text-[#e2e2f0] mb-1">Otakudesu Direct Scraper</h1>
      <p class="text-xs text-[#7c7c9a]">Gunakan halaman ini untuk melakukan debugging dan mengekstrak tautan video dari Otakudesu secara langsung.</p>
    </div>

    <!-- Input Card -->
    <div class="bg-[#13131e] border border-[#1f1f2e] rounded-lg p-5 mb-6">
      <div class="flex flex-col gap-3">
        <label class="text-xs text-[#7c7c9a] font-semibold">URL Halaman Episode Otakudesu</label>
        <div class="flex gap-2">
          <input
            type="text"
            placeholder="Contoh: https://otakudesu.blog/episode/solo-leveling-season-2-episode-13-sub-indo/"
            v-model="targetUrl"
