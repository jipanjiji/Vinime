    ...
    try {
      ...
      if (slug) {
        ...
        if (hashData?.success) {
          ...
          if (hashData.hash) {
            // Use server-side proxy to avoid browser CORS block on animeku.org
...
<template>
  <div class="max-w-4xl mx-auto px-4 py-8">
    <!-- Header -->
    <div class="mb-6">
      <h1 class="text-xl font-bold text-[#e2e2f0] mb-1">Kuronime Direct Scraper</h1>
      <p class="text-xs text-[#7c7c9a]">Gunakan halaman ini untuk melakukan debugging dan mengekstrak tautan video dari Kuronime secara langsung.</p>
    </div>

    <!-- Input Card -->
    <div class="bg-[#13131e] border border-[#1f1f2e] rounded-lg p-5 mb-6">
      <div class="flex flex-col gap-3">
        <label class="text-xs text-[#7c7c9a] font-semibold">URL Halaman Episode Kuronime</label>
        <div class="flex gap-2">
          <input
            type="text"
            placeholder="Contoh: https://kuronime.sbs/nonton-solo-leveling-season-2-episode-13/"
            v-model="targetUrl"
